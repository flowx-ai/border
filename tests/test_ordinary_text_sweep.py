...
@pytest.mark.xfail(
    reason=(
        ...
        "`Grelmshof` scores 0.97, so it is unfamiliarity and not any knowledge of place "
        "names. Sentence-initial position is exempt, because capitalisation there is "
        "orthography.\n\n"
        "`options.entity_thresholds: {person: 0.90}` removes 30 of 43 such findings. It "
        "costs nothing measurable: held-out PERSON recall is 1.0000 over 668 gold "
        "spans, 0 of 560 gold spans survive verbatim in the redacted output, and 14 "
        "hand-written people across 8 scripts all clear it. 0.95 removes three more and "
        ...
        "What is left: `person` 13 spans, `phone` 4, `national_id` 4, plus nsfw and "
        "injection rows. Most of the surviving `person` findings are places named after "
        "people, `Franjo Tudman` at 0.977 and `Deak Ferenc` at 0.962, where the span "
        "does contain a person's name. Still counted as damage, because a caller who "
...
@pytest.mark.xfail(
    reason=(
        ...
        "which is 0.0769. A date is still found and still recorded; it is no longer cut "
