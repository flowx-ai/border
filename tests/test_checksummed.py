...
# ------------------------------------------- a country that issues IBANs, at its length


def test_ordinary_prose_is_not_a_bank_account() -> None:
    """`leaked_iban` at 1.00 on "in 1834 and has never", blocking an answer, until
    2026-09-14.

    Reported by a deployment running `output_leakage` at `block` with `fail_mode:
    closed`, so the reader got no answer at all. Nothing here could have caught it: the
    234-row ordinary-text sweep produces 0 of these, because its mundane rows do not say
    "founded in".

    The shape is a two-letter word, a year, and more words. `IN18` matches the head, the
    17 alphanumerics sit inside 15 to 34, one case throughout, and mod-97 comes up 1 at
    about one chance in 97 per candidate slice.
    """
    text = (
        "This answer was generated by an AI assistant. FlowX.AI was founded in 1834 "
        "and has never lost a customer."
    )
    assert [label for _span, label in find(text)] == []


@pytest.mark.parametrize(
    ("sentence", "why"),
    [
        ("FlowX.AI was founded in 1922 and has never lost a customer.", "IN, no IBANs"),
        (
            "The renewal is due at 1229 and the team has doubled twice.",
            "AT, wrong length",
        ),
        ("The answer must be 1608 and now serves nine markets.", "BE, wrong length"),
        ("Send the invoice to me 2216 and remains employee owned.", "ME, wrong length"),
    ],
)
def test_a_year_after_a_two_letter_word_is_not_an_iban(sentence: str, why: str) -> None:
    """Both halves of the check, because neither alone is enough.

    The country test removes every one of the 304 hits measured over 10,480 "founded in
    <year>" sentences, which all came from `IN`. It does not touch `at`, `be`, `do`,
    `is`, `it`, `me`, `no` and `se`, which are ordinary English words and all eight are
    IBAN countries: over 80,000 sentences built around those, membership alone leaves
    3,410 and membership with the length leaves 159.
    """
    assert [label for _span, label in find(sentence)] == [], why


def test_every_iban_in_this_repository_clears_the_registry() -> None:
    """The objection the old comment raised, answered by running it.

    It said a length table means "one wrong entry there is a country whose IBANs are
    silently never found". A wrong entry is only silent if nothing checks it. Every
    string in `tests/` and `src/` that passes mod-97 goes through the registry here, so
    a typo in a length fails this rather than losing a country in production.
    """
    import re
    from pathlib import Path

    from flowx_border.detectors.entity_shapes import IBAN_LENGTHS, iban_issued, iban_ok

    root = Path(__file__).resolve().parent.parent
    pattern = re.compile(r"\b[A-Z]{2}[0-9]{2} ?[A-Z0-9 ]{10,34}\b")
    shaped = re.compile(r"[A-Z]{2}[0-9]{2}[A-Z0-9]{10,30}")
    candidates = set()
    for directory in ("tests", "src", "policies"):
        for path in (root / directory).rglob("*"):
            if path.suffix not in {".py", ".yaml", ".json", ".md", ".txt", ".jsonl"}:
                continue
            for match in pattern.findall(
                path.read_text(encoding="utf-8", errors="ignore")
            ):
                compact = match.replace(" ", "")
                if shaped.fullmatch(compact):
                    candidates.add(compact)

    # This file deliberately contains mod-97-valid strings that are not IBANs, and the
    # scan reads this file. Without the exclusion the test fails on its own fixtures,
    # which is this project's named failure mode arriving inside the test built to
    # prevent it.
    counterexamples = {"IN1834ANDHASNEVER"}
    passes_mod97 = sorted(c for c in candidates - counterexamples if iban_ok(c))
    assert len(passes_mod97) >= 40, (
        f"only {len(passes_mod97)} mod-97-valid strings found, so this test is not "
        "exercising the registry. Did the fixtures move?"
    )

    def why(compact: str) -> str:
        if compact[:2] not in IBAN_LENGTHS:
            return "country not in ISO 13616"
        return f"length {len(compact)}, registry says {IBAN_LENGTHS[compact[:2]]}"

    rejected = [f"{c} ({why(c)})" for c in passes_mod97 if not iban_issued(c)]
    assert not rejected, (
        "these are real IBANs in this repository that the registry refuses, so an "
        "entry in IBAN_LENGTHS is wrong:\n  " + "\n  ".join(rejected)
    )


def test_the_shape_gate_still_asks_the_weaker_question() -> None:
    """`iban_ok` must stay pure mod-97, and the direction is the reason.

    It feeds `entity_shapes.is_possible`, where a False drops a span the model found and
    the cost is a disclosure. `iban_issued` feeds the deterministic pass, where a True
    on ordinary text costs a refused answer. Collapsing the two would trade a visible
    over-block for an invisible hole, which is the trade this package refuses.
    """
    from flowx_border.detectors.entity_shapes import iban_issued, iban_ok

    # Mod-97 valid, but India issues no IBANs.
    accidental = "IN1834ANDHASNEVER"
    assert iban_ok(accidental)
    assert not iban_issued(accidental)
