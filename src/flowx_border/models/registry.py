...
UNPUBLISHED: Final[dict[str, str]] = {
    ...
    "groundedness": (
        ...
        "not be published as it stands. It generalises inside its generator's style and"
        " not outside it, which its own test split cannot detect because that split "
        "came from the same generator.\n\n"
        "Measured 2026-08-12. On the test split it scores 0.882 exact-match accuracy "
        "overall and 0.864 on the paraphrase register, which is the register designed "
        "to defeat word overlap and which genuinely does: those examples share 0.165 of"
        " their content words with their source, against 0.984 for verbatim support. So"
        " the corpus is well constructed and the model is not merely counting"
        " words.\n\n"
        "On hand-written probes in the same domain, against a 500 character source of "
        "the length it was trained on, it reads a paraphrase of a stated fact as "
        "contradicted at 0.0013, and a strictly weaker claim the source fully supports "
        "as contradicted. It reads a near-verbatim restatement as supported at 0.9999, "
        "an invention as unsupported and a real contradiction as contradicted. So it "
        "has learned this generator's way of paraphrasing rather than paraphrase, and "
        "an LLM answer paraphrases its sources in its own way.\n\n"
        "Two earlier readings of this were wrong and are recorded because the shape of "
        "the mistake keeps recurring. The first said the model scores string similarity"
        " rather than entailment, which the paraphrase and lexical_overlap registers "
        "disprove. The second said the supported class had been generated as near-"
        "copies, which the overlap measurement disproves. Both were plausible stories "
        "about a number rather than readings of it.\n\n"
        "The fix is a corpus generated with stylistic variety, and an evaluation set "
        "produced some other way, because a test split drawn from the training "
        "generator cannot measure this. Weakest registers in-distribution are "
        "negation_contradiction at 0.754 and multi_sentence_support at 0.803."
    ),
